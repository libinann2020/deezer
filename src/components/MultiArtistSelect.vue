<template>
    <div>
        <RouterLink :to="to" class="relative">
            <div class="flex justify-center items-center contrast-[2] brightness-[0.87] rounded-md aspect-square overflow-hidden" @mouseenter="isHover = true" @mouseleave="isHover = false">
                <div :class="isHover ? 'transition ease-in duration-150 bg-opacity-20' : 'transition ease-out duration-150 bg-opacity-5'" class="absolute w-full h-full bg-black z-10 rounded-md" />
                ...
                <div class="absolute z-30 flex items-center bottom-1 left-0 h-7 ml-1 bg-black rounded-full px-2 bg-opacity-10">
                    ...
                    <div class="text-white font-bold text-[12px]">{{ category }}</div>
                </div>
            </div>
        </RouterLink>
        <div class="h-2"></div>
